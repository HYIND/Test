...
void FileTransferDownLoadTask::RecvChunkDataAndAck(const json &js)
{
    bool error = false;
...
void FileTransferDownLoadTask::ProcessMsg(const Buffer &buf)
{
    json js;
    try
    {
...
void FileTransferDownLoadTask::ProcessMsg(const json &js)
{
    ...
    if (js.contains("command"))
    {
        ...
        if (IsFinished)
        {
            ...
            if (!GetSession()->AsyncSend(buf_data))
                OccurError();
            return;
        }
